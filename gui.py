...
class NovaLunaGUI:
    def __init__(self, root):
        ...
        self.game = Game(4)  # Ahány játékossal szeretnénk játszani 1-4 között
        self.selected_card = None
        self.selected_card_position = None
        self.available_positions = []  # Elérhető pozíciók kártya húzás után
        self.inventory_window = None  # Inventory ablak nyomon követése
        self.create_widgets()
        self.update_board()
        self.update_info()
    ...
    def draw_card_board(self):
        ...
        for i in range(len(self.game.card_board)):
            ...
            if i == self.game.moon_marker_position:
                ...
                if isinstance(card, str):  # A stringeket kihagyja szóval a moon markert
                    continue
    ...
    def on_card_click(self, card_position):
        ...
        if card_position in self.available_positions:
            if self.selected_card is not None and self.selected_card_position is not None:
                self.game.card_board[self.selected_card_position] = self.selected_card  # Vissza teszi az elöző kiválasztott kártyát
    ...
    def open_inventory_window(self):
        # Csak akkor csinál újjat ha kell
    ...
    def draw_inventory(self, inventory_frame, inventory_canvas):
        inventory_canvas.delete("all")  # Töröljük mielött újra rajzoljuk a canvas-t hogy ne clippeljenek a dolgok
        current_player = self.game.players[self.game.current_player_index]
        size = 60  # A méret beállítása
        margin = 5
    ...
    def on_inventory_click(self, grid_position, inventory_canvas):
        ...
        if self.is_valid_placement(player, x, y):
            try:
                ...
                self.selected_card = None
                self.selected_card_position = None
                inventory_canvas.update_idletasks()  # Frissítjűk az inventory-t miután leteszünk egy kártyát
                self.update_inventory()  # Ablak frissítés
                self.update_board()
    ...
    def auto_place_first_card(self):
        # Mivel sok stratégia nincs az első lehelyezett kártyához ezt automatán tesszük le
        current_player = self.game.players[self.game.current_player_index]
        if len(current_player.inventory.get_all_cards()) == 0:
            ...
            print(f"Placing first card at ({center_x}, {center_y})")
            ...
            self.selected_card = None
            self.selected_card_position = None
            self.update_board()  # Fríssitjük a boardot hogy mozogjon a moon marker
    ...
    def check_token_completion(self, player, card, token, x, y):
        ...
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            if (nx, ny) in player.inventory.grid:
                neighbor_card = player.inventory.get_card(nx, ny)
                if neighbor_card:
                    counts[neighbor_card.color] += 1
        for color, count in token.__dict__.items():
            if count and count > counts[color]:
                return
